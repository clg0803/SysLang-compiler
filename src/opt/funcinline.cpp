...
std::vector<std::string> FI::FuncInline::do_func_inline()
{
    ...
    for (auto f_name : func_name)
    {
        ...
        for (auto f_p : fun_pos)
        {
            ...
            std::unordered_map<int, int> tmp_var_mp;               // callee 中的临时变量   ->  caller 中的临时变量
            std::unordered_map<std::string, std::string> label_mp; // callee 中的标签       ->  caller 中的标签
            assert(ill);
            // do empty pram copy
...
/**
 * 得到函数的基本属性：
 * 1. void recur stack_szie code_len
 * 2. 被调用次数
 * 3. 调用其他函数的次数 / 调用的函数名->位置(语句)
 * 4. 使用到的全局变量(包括本函数中的和调用的函数中的、通过ldr/str操作的都视为全局变量)
*/
