...
LIR::Exp *ConstExp::dCopy() { return new ConstExp(val); }
LIR::Exp *BinOpExp::dCopy() { return new BinOpExp(this->binop, l->dCopy(), r->dCopy()); }
LIR::Exp *TempVarExp::dCopy() { return new TempVarExp(this->temp_label_id_int); }
LIR::Exp *MemExp::dCopy() { return new MemExp(mem_addr->dCopy()); }
LIR::Exp *NameExp::dCopy() { return new NameExp(name); }
LIR::Exp *LIR::CallExp::dCopy()
{
    std::vector<LIR::Exp *> nwa;
    for (auto it : args)
        nwa.push_back(it->dCopy());
    return new LIR::CallExp(func_name_str, nwa);
}
